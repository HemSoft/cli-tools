...
function Find-N8nDataVolume {
    # Tries to find a named docker volume that likely contains existing n8n data
    # Heuristic: volume root contains database.sqlite (SQLite default)
    # Uses busybox for a tiny probe container
    Write-Host "Scanning existing Docker volumes for n8n data..." -ForegroundColor Yellow
    $volumes = docker volume ls --format "{{.Name}}"
    if (-not $volumes) { return $null }

    $volList = $volumes -split "`n" | Where-Object { $_.Trim() -ne '' }
    foreach ($vol in $volList) {
        # Skip well-known non-n8n volumes quickly if desired (none by default)
        # Probe for database.sqlite at the root (typical for /home/node/.n8n mapping)
        $null = docker pull busybox 2>$null | Out-Null
        $probeCmd = @('run','--rm','-v',"${vol}:/data",'busybox','sh','-lc','test -f /data/database.sqlite')
        & docker @probeCmd | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Found existing n8n SQLite data in volume '$vol'" -ForegroundColor Green
            return $vol
        }
    }
    Write-Host "No existing n8n data volume detected by heuristic." -ForegroundColor Yellow
    return $null
}
...
if ($containerExists -and $currentConfig) {
    ...
    # Try to auto-discover an existing data volume before falling back
    $detectedDataVol = Find-N8nDataVolume
    $volumeToUse = if ($detectedDataVol) { $detectedDataVol } else { $defaultVolume }

    if (-not $detectedDataVol) {
        # Ensure the fallback default exists or will be created
        ...
        Write-Host "Proceeding with default volume mapping: ${defaultVolume}:/home/node/.n8n" -ForegroundColor Yellow
    } else {
        Write-Host "Proceeding with discovered data volume mapping: ${detectedDataVol}:/home/node/.n8n" -ForegroundColor Green
    }

    # Default: port 5678, data at /home/node/.n8n, restart always
    $args = @('run','-d','--name', $containerName,'--restart','always','-p','5678:5678','-v',"${volumeToUse}:/home/node/.n8n", $imageToUse)
